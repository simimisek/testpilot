...
export async function POST(req: Request) {
  try {
    ...
    if (!url || !serviceKey) {
      return NextResponse.json({ error: 'Chybí env proměnná SUPABASE_SERVICE_ROLE_KEY nebo NEXT_PUBLIC_SUPABASE_URL' }, { status: 500 })
    }
    ...
    if (profileError) return NextResponse.json({
      error: 'Uživatel vytvořen, ale nepodařilo se nastavit profil: ' + profileError.message
    }, { status: 500 })
    ...
    return NextResponse.json({ error: 'Neočekávaná chyba: ' + msg }, { status: 500 })
  }
}
